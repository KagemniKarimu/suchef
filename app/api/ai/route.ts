...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, message, context, mode } = body
    ...
    if (!openai) {
      const mockResponse = getRandomResponse((mode || 'learn') as ResponseCategory)
      await new Promise(resolve => setTimeout(resolve, 500))
      
      return NextResponse.json({
        message: mockResponse,
        ...(mode === 'review' && { rating: Math.floor(Math.random() * 2) + 3.5 }),
        ...(mode === 'nyanya' && { 
    ...
    // Real OpenAI call
    const systemPrompt = prompt || AI_PROMPTS[mode as AIMode] || AI_PROMPTS.learn
    ...
    const aiMessage = completion.choices[0].message.content || "Sorry, one more time?"
